...
for n in 2 4 6 8 10 12 14 16; do
    for c in ${sel_list[*]}; do
        ...
        ${timeout_prefix} ./benchmark.out FREE -t 1 -w ${wl_num} -o ${dirname} -n ${n} --presuf -c ${c} -e ${num_repeat}
        for t in ${thread_list[*]}; do
            echo benchmark.out FREE -t ${t} -w ${wl_num} -o ${dirname} -n ${n} -c ${c} -e ${num_repeat}
            ${timeout_prefix} ./benchmark.out FREE -t ${t} -w ${wl_num} -o ${dirname} -n ${n} -c ${c} -e ${num_repeat}
        done
    done
done
echo benchmark.out FAST -t 1 -w ${wl_num} -o ${dirname}  --relax DETERM -e ${num_repeat}
${timeout_prefix} ./benchmark.out FAST -t 1 -w ${wl_num} -o ${dirname} --relax DETERM -e ${num_repeat}
echo benchmark.out FAST -t 1 -w ${wl_num} -o ${dirname}  --relax RANDOM -e ${num_repeat}
${timeout_prefix} ./benchmark.out FAST -t 1 -w ${wl_num} -o ${dirname} --relax RANDOM -e ${num_repeat}
