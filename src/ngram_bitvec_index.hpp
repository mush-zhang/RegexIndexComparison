...
class NGramBitvecIndex {
 public:
    ...
    void print_index(bool size_only=false) const;
    
    const std::vector<size_t> & get_line_pos_at(const std::string & key) const;
    ...
    bool empty() const { return k_index_keys_.empty(); }

    size_t get_num_keys() const { return k_index_keys_.size(); }

    size_t get_bytes_used() const;
