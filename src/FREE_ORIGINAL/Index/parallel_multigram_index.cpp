...
void free_original_index::ParallelMultigramIndex::add_or_inc_w_lock(
...
void free_original_index::ParallelMultigramIndex::get_uni_bigram(size_t idx,
...
void free_original_index::ParallelMultigramIndex::insert_unigram_into_index(
...
void free_original_index::ParallelMultigramIndex::insert_bigram_into_index(
...
void free_original_index::ParallelMultigramIndex::get_kgrams_not_indexed(size_t idx,
...
void free_original_index::ParallelMultigramIndex::insert_kgram_into_index(
...
void free_original_index::ParallelMultigramIndex::select_grams(int upper_n) {
    ...
    for (int i = 0; i < thread_count_; i++) {
        threads.push_back(std::thread(
            &free_original_index::ParallelMultigramIndex::get_uni_bigram, this,
    ...
    for (; i < thread_count_ && num_per_thread*(i+1) < unigrams.size(); i++) {
        ...
        threads.push_back(std::thread(
            &free_original_index::ParallelMultigramIndex::insert_unigram_into_index, this,
    ...
    for (; i < thread_count_ && num_per_thread*(i+1) < bigrams.size(); i++) {
        ...
        threads.push_back(std::thread(
            &free_original_index::ParallelMultigramIndex::insert_bigram_into_index, this,
    ...
    while (!expand.empty() && k <= upper_n && 
        ...
        for (int i = 0; i < thread_count_; i++) {
            threads.push_back(std::thread(
                &free_original_index::ParallelMultigramIndex::get_kgrams_not_indexed, this,
        ...
        for (; i < thread_count_ && num_per_thread*(i+1) < curr_kgrams.size(); i++) {
            ...
            threads.push_back(std::thread(
                &free_original_index::ParallelMultigramIndex::insert_kgram_into_index, this,
...
void free_original_index::ParallelMultigramIndex::kgrams_in_line(int upper_n, size_t idx,
...
void free_original_index::ParallelMultigramIndex::merge_lists(
...
void free_original_index::ParallelMultigramIndex::fill_posting(int upper_n) {
    std::vector<std::thread> threads;
    std::vector<std::unordered_map<std::string, std::vector<size_t>>> loc_idxs(thread_count_);
    for (int i = 0; i < thread_count_; i++) {
        threads.push_back(std::thread(
            &free_original_index::ParallelMultigramIndex::kgrams_in_line, this,
    ...
    for (; i < thread_count_ && num_per_thread*(i+1) <= k_index_keys_.size(); i++) {
        std::advance(end_it, num_per_thread);
        threads.push_back(std::thread(
            &free_original_index::ParallelMultigramIndex::merge_lists, this,
...
template 
void free_original_index::ParallelMultigramIndex::add_or_inc_w_lock<char, std::hash<char>>(
        std::map<char, atomic_ptr_t> & kgrams, 
        const char & key, std::unordered_set<char, std::hash<char>> & loc_visited_kgrams);

template 
void free_original_index::ParallelMultigramIndex::add_or_inc_w_lock<std::pair<char,char>, hash_pair>(
...
template 
void free_original_index::ParallelMultigramIndex::add_or_inc_w_lock<std::string, std::hash<std::string>>(
        std::map<std::string, atomic_ptr_t> & kgrams, 
